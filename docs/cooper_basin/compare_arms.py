...
def main(argv=None):
    ...
    obs = sf.observed()
    P_REF = fp.datum()
    print(f"datum {P_REF:.3f} MPa at the wellhead (p_f0 {fp.P_F0}, rho "
          f"{fp.RHO_FLOW:.0f}, column {fp.Z_COLUMN:.1f} m); friction "
          f"{fp.friction(60.9):.2f} MPa at peak")
    ...
    for arm in a.arms:
        ...
        adp.plot(_to, _dpo, lw=1.2, color=MUTED, alpha=0.85,
